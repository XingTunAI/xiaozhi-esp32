...
历史对照（未通过，已撤回）：Voice Lab standalone 的 Wi-Fi STA 采用 20 MHz 带宽；修改只作用于设备，不改动路由器。2026-09-10 的 40 MHz 实测出现频繁 TCP 重传与发送停顿，此配置需要重新做实机持续录音验收，不能仅据理论参数宣称修复。
...
历史两帧合包对照（未通过，已撤回）：实时发送继续采到即发；网络卡顿恢复时，每包最多带两帧已经排队的 PCM（40 ms，VLA2 包 1320 字节），不等待凑包，以减少积压恢复阶段的包数。录音总 RAM 上限保持四秒。

当前交付候选恢复 Wi-Fi 默认协商带宽和单帧 20 ms PCM 直接发送。保留新帧优先与每轮一帧的有界重传。当前网络仍存在长达 13.244 秒的 socket 发送停顿，连续录音尚未通过验收。
