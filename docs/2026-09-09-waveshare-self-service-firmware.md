...
| K2 录音中短按 | 不改变录音状态，不播放提示音（2026-09-10 移除暂停/继续） |
| K2 长按约 2 秒 | 结束当前录音会话，发送尾部音频，等待服务端确认；设备停止和文本整理分别表示 |
| BOOT 长按 3 秒 | 进入重新配网；不清设备凭据和正式服务入口 |
| BOOT 短按 | 配网中重播提示和热点密码；其他状态不改变录音或触发配网 |
| 无已保存网络 | 自动进入配网，播报提示和本机独立密码 |
| 配网热点 | `XingTun-Audio-XXXX`，WPA2，独立随机 8 位数字密码；配网等待 5 分钟后尝试旧网络；无旧配置继续保留 AP，旧网络 60 秒连不上则重新开放 AP |
...
- 手机错误密码、正确配网、5 分钟旧网回退、无旧网保留 AP、提交中不关闭、长按重开，以及换网后设备身份保持。
...
### 历史方案：同一会话暂停 / 继续（2026-09-10 已移除）

以下为已停用的历史方案，当前以板型 README 为准。原 K2 操作为长按约 2 秒开始 / 结束，短按暂停 / 继续；重新配网保留 BOOT 长按 3 秒。
...
### 2026-09-10 网页录音再次中断：修复录音省电生命周期

71 秒管理端及 K2 暂停继续的有限测试之后，网页再次中断。复现实机日志显示 16040 字节的
半秒 PCM 包发送耗时接近两秒，生产队列持续增长，最终四秒上限也溢出。stale_revision 是
停采后重试指令触发的连带报错，不能代替初始故障原因。

独立 Voice Lab 没有经过小智协议音频通道的性能模式切换，Wi-Fi 仍处于待机 MAX_MODEM 省电。
修复让请求、连接、录音、暂停、收尾期间保持 PERFORMANCE，在 Ready/Error 恢复 LOW_POWER；
清理旧播放后再次确保性能模式，避免 StopNotification 将其改回省电。发送超过 500 ms 时
记录实际耗时和包大小，维持现有缓存上限，不将增加容量作为本次修复。
此项需要新构建及实机验证，不能据代码变更预先宣称网络性能已经恢复。
