...
| K2 长按约 2 秒 / BOOT 短按 | 结束当前录音或暂停的会话，发送尾部音频，等待服务端确认；设备停止和文本整理分别表示 |
...
管理员网页开始、K2 长按 / BOOT 短按结束也需要准确关联实验。固件在收到开始授权时保存 `recordingId = desired_config.test.correlationId` 与 `recordingRevision = 开始指令 revision`，并在以下状态中返回同一快照：
