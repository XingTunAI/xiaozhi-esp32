...
public:
    ...
    void SetCustomerMode(bool enabled) { customer_mode_ = enabled; }
    void SetPassword(const std::string& password) { ap_password_ = password; }
    void Start();
    void Stop();
...
private:
    ...
    void StartWebServer();
    void RequestExit(uint32_t delay_ms);
