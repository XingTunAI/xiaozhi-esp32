...
std::string NormalizeVoiceLabEnrollmentCode(const char* code) {
...
void SaveVoiceLabEnrollmentCodeIfPresent(cJSON* json) {
    cJSON* item = cJSON_GetObjectItemCaseSensitive(json, "voice_lab_enrollment_code");
    if (!cJSON_IsString(item) || item->valuestring == nullptr) {
        return;
    }

    auto code = NormalizeVoiceLabEnrollmentCode(item->valuestring);
    if (code.empty()) {
        return;
    }
    if (code.size() != 9 || code[4] != '-') {
        ESP_LOGW(TAG, "Ignoring invalid Voice Lab enrollment code from WiFi config page");
        return;
    }

    nvs_handle_t nvs;
    esp_err_t err = nvs_open("voice_lab", NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open Voice Lab NVS for enrollment code: %s", esp_err_to_name(err));
        return;
    }

    nvs_set_str(nvs, "enrollment_code", code.c_str());
    nvs_erase_key(nvs, "device_token");
    nvs_erase_key(nvs, "token");
    err = nvs_commit(nvs);
    nvs_close(nvs);

    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Voice Lab enrollment code stored from WiFi config page");
    } else {
        ESP_LOGE(TAG, "Failed to save Voice Lab enrollment code: %s", esp_err_to_name(err));
    }
}

void SaveVoiceLabServerConfigIfPresent(cJSON* json) {
    cJSON* host_item = cJSON_GetObjectItemCaseSensitive(json, "voice_lab_server_host");
    cJSON* port_item = cJSON_GetObjectItemCaseSensitive(json, "voice_lab_server_port");
    cJSON* tls_item = cJSON_GetObjectItemCaseSensitive(json, "voice_lab_server_tls");
    cJSON* device_key_item = cJSON_GetObjectItemCaseSensitive(json, "voice_lab_device_key");

    const bool has_host = cJSON_IsString(host_item) && host_item->valuestring != nullptr &&
                          host_item->valuestring[0] != '\0';
    const bool has_port =
        cJSON_IsNumber(port_item) && port_item->valueint > 0 && port_item->valueint <= 65535;
    const bool has_tls = cJSON_IsBool(tls_item);
    const bool has_device_key = cJSON_IsString(device_key_item) &&
                                device_key_item->valuestring != nullptr &&
                                device_key_item->valuestring[0] != '\0';
    if (!has_host && !has_port && !has_tls && !has_device_key) {
        return;
    }
    ...
    esp_err_t err = nvs_open("voice_lab", NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open Voice Lab NVS for server config: %s", esp_err_to_name(err));
        return;
    }

    if (has_host) {
        nvs_set_str(nvs, "server_host", host_item->valuestring);
        nvs_erase_key(nvs, "server_url");
    }
    if (has_port) {
        nvs_set_i32(nvs, "server_port", port_item->valueint);
    }
    if (has_tls) {
        nvs_set_u8(nvs, "server_tls", cJSON_IsTrue(tls_item) ? 1 : 0);
    }
    if (has_device_key) {
        nvs_set_str(nvs, "device_key", device_key_item->valuestring);
        nvs_erase_key(nvs, "device_token");
        nvs_erase_key(nvs, "token");
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Voice Lab server config stored from WiFi config page");
    } else {
        ESP_LOGE(TAG, "Failed to save Voice Lab server config: %s", esp_err_to_name(err));
    }
}
...
extern const char customer_html_start[] asm("_binary_wifi_customer_configuration_html_start");
...
void WifiConfigurationAp::Start() {
    ...
    esp_timer_create_args_t timer_args = {.callback =
                                              [](void* arg) {
                                                  auto* self =
                                                      static_cast<WifiConfigurationAp*>(arg);
...
void WifiConfigurationAp::StartAccessPoint() {
    ...
    wifi_config.ap.authmode = ap_password_.empty() ? WIFI_AUTH_OPEN : WIFI_AUTH_WPA2_PSK;
    if (!ap_password_.empty()) {
        strlcpy(reinterpret_cast<char*>(wifi_config.ap.password), ap_password_.c_str(),
...
void WifiConfigurationAp::StartWebServer() {
    ...
    httpd_uri_t index_html = {.uri = "/",
                              ...
                              .handler = [](httpd_req_t* req) -> esp_err_t {
                                  ...
                                  const char* page =
                                      self->customer_mode_ ? customer_html_start : index_html_start;
    ...
    httpd_uri_t scan = {
        ...
        .handler = [](httpd_req_t* req) -> esp_err_t {
            ...
            for (int i = 0; i < this_->ap_records_.size(); i++) {
                ...
                cJSON_AddNumberToObject(item, "rssi", this_->ap_records_[i].rssi);
                cJSON_AddNumberToObject(item, "authmode", this_->ap_records_[i].authmode);
                char* encoded = cJSON_PrintUnformatted(item);
                cJSON_Delete(item);
                if (!encoded) {
                    return ESP_ERR_NO_MEM;
                }
                httpd_resp_sendstr_chunk(req, encoded);
                cJSON_free(encoded);
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t* req) -> esp_err_t {
            ...
            while (received < buf_len) {
                const int ret = httpd_req_recv(req, buf + received, buf_len - received);
                if (ret <= 0) {
                    free(buf);
            ...
            // 解析 JSON 数据
            cJSON* json = cJSON_ParseWithOpts(buf, nullptr, true);
            free(buf);
            ...
            if (this_->customer_mode_) {
                ...
                cJSON_ArrayForEach (field, json) {
                    if (!field->string || (strcmp(field->string, "ssid") != 0 &&
                                           strcmp(field->string, "password") != 0)) {
                        cJSON_Delete(json);
                        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST,
                                            "Only WiFi settings are accepted");
                        return ESP_OK;
                    }
                }
            }
            ...
            if (!cJSON_IsString(ssid_item) || (ssid_item->valuestring == NULL) ||
                strlen(ssid_item->valuestring) == 0 || (strlen(ssid_item->valuestring) >= 33)) {
                cJSON_Delete(json);
            ...
            if (this_->customer_mode_ &&
                (!cJSON_IsString(password_item) || !password_item->valuestring ||
                 strlen(password_item->valuestring) > 64)) {
                cJSON_Delete(json);
            ...
            if (!this_->ConnectToWifi(ssid_str, password_str)) {
                cJSON_Delete(json);
                httpd_resp_send(
                    req, "{\"success\":false,\"error\":\"Failed to connect to the Access Point\"}",
                    HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }
            ...
            if (!this_->customer_mode_) {
                SaveVoiceLabServerConfigIfPresent(json);
                SaveVoiceLabEnrollmentCodeIfPresent(json);
            }
            cJSON_Delete(json);
            // 设置成功响应
            httpd_resp_set_type(req, "application/json");
            ...
            if (this_->customer_mode_) {
                // Closing the browser must not leave a successful setup AP open.
                this_->RequestExit(2000);
            }
            return ESP_OK;
        },
        .user_ctx = this};
    ...
    httpd_uri_t exit_config = {.uri = "/exit",
                               .method = HTTP_POST,
                               .handler = [](httpd_req_t* req) -> esp_err_t {
                                   auto* this_ = static_cast<WifiConfigurationAp*>(req->user_ctx);
    ...
    // Customer firmware has no engineering endpoints, even for direct requests.
    if (customer_mode_) {
        ESP_LOGI(TAG, "Customer WiFi portal started");
        return;
    }
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t* req) -> esp_err_t {
            ...
            ESP_LOGI(
                TAG,
                "Saved settings: ota_url=%s, max_tx_power=%d, remember_bssid=%d, sleep_mode=%d",
                this_->ota_url_.c_str(), this_->max_tx_power_, this_->remember_bssid_,
                this_->sleep_mode_);
            return ESP_OK;
        },
        .user_ctx = this};
...
void WifiConfigurationAp::RequestExit(uint32_t delay_ms) {
    if (exit_pending_.exchange(true)) {
        return;
    }
    struct ExitContext {
    ...
    auto* context = new (std::nothrow) ExitContext{this, config_generation_.load(), delay_ms};
    if (!context) {
        exit_pending_ = false;
        return;
    }
    const auto created = xTaskCreate(
        [](void* arg) {
            std::unique_ptr<ExitContext> context(static_cast<ExitContext*>(arg));
            vTaskDelay(pdMS_TO_TICKS(context->delay_ms));
            auto* self = context->self;
            if (self->config_generation_.load() == context->generation &&
        ...
        "exit_config_task", 4096, context, 5, nullptr);
    if (created != pdPASS) {
        delete context;
        exit_pending_ = false;
        ESP_LOGE(TAG, "Could not schedule configuration exit");
    }
}
...
bool WifiConfigurationAp::ConnectToWifi(const std::string& ssid, const std::string& password) {
    ...
    for (int attempt = 1; attempt <= kMaxAttempts && !connected; ++attempt) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "esp_wifi_connect failed: %d (attempt %d/%d)", ret, attempt,
                     kMaxAttempts);
            continue;
        }
        ESP_LOGI(TAG, "Connecting to WiFi %s (attempt %d/%d)", ssid.c_str(), attempt, kMaxAttempts);

        // Wait for the connection to complete for 10 or 25 seconds.
        EventBits_t bits =
            xEventGroupWaitBits(event_group_, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdTRUE, pdFALSE,
#ifdef CONFIG_SOC_WIFI_SUPPORT_5G
                                pdMS_TO_TICKS(25000)
#else
                                pdMS_TO_TICKS(10000)
        ...
        if (bits & WIFI_CONNECTED_BIT) {
            ...
            ESP_LOGW(TAG, "Attempt %d/%d %s%s", attempt, kMaxAttempts,
    ...
    if (connected) {
        ...
        ESP_LOGE(TAG, "Failed to connect to WiFi %s after %d attempts", ssid.c_str(), kMaxAttempts);
        return false;
    }
}
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id,
                                           void* event_data) {
...
void WifiConfigurationAp::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id,
                                         void* event_data) {
    WifiConfigurationAp* self = static_cast<WifiConfigurationAp*>(arg);
...
void WifiConfigurationAp::StartSmartConfig() {
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        SC_EVENT, ESP_EVENT_ANY_ID, &WifiConfigurationAp::SmartConfigEventHandler, this,
        &sc_event_instance_));
    ...
    if (event_base == SC_EVENT) {
        switch (event_id) {
            ...
            case SC_EVENT_GOT_SSID_PSWD: {
                ...
                ESP_LOGI(TAG, "SmartConfig credentials received");
                // 尝试连接WiFi会失败，故不连接
